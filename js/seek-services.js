...
document.getElementById("prodSearch").addEventListener("input", () => {
    ...
    fetch("https://pragati-one.herokuapp.com/services/search?q=" + document.getElementById("prodSearch").value)
        .then((response) => response.json())
        .then((res) => {
            res = res.message;
...
fetch("https://pragati-one.herokuapp.com/services/getall", {
    method: "GET",
    headers: {
...
function registerInterest(event) {
  ...
  document.getElementById(`interest-cover-${prodId}`).appendChild(div);
    ...
    fetch('https://pragati-one.herokuapp.com/services/interested', {
        method: 'POST',
        headers: {
            "Content-Type": "application/json",
            Authorization: token,
